...
- (void)testDynamicFields
{
    ...
    // Tapping "ADD" in a disabled data source shouldn't do anything
    NSInteger numberOfFields = [dataSource numberOfFields];

    // "tapping" the autogenerated add cell will trigger the add
    // logic, after this magic should happen.
    [dataSource fieldCell:nil updatedWithField:field];
    XCTAssertEqual(numberOfFields + 1, [dataSource numberOfFields]);

    // test that one section was added bellow the dynamic companies section.
    FORMSection *section = [dataSource sectionWithID:@"companies[0]"];
    XCTAssertNotNil(section);

    // test that this new section has the template fields
